...
class Infrastructure extends EventEmitter {
  constructor () {
    ...
    this._representation.devices.forEach(async (device, deviceIndex) => {
      ...
      doc.nodes.forEach((node, nodeIndex) => {
        ...
        this._representation.devices[deviceIndex].nodes[deviceIndex].state = node.state;
      });
    });
  ...
  sendProperty (options) {
    ...
    mqtt.publish(`devices/${deviceId}/${nodeId}/${property}/set`, value.toString(), { qos: qos, retain: retain });
  }
