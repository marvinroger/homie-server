...
class DataDirValidator {
  ...
  validateInfrastructure (object) {
    ...
    if (!validate(object)) {
      ...
      log.error('infrastructure.json invalid');
      return false;
    }
    ...
    if ((new Set(deviceIds)).size !== deviceIds.length) {
      log.error('infrastructure.json contains duplicate devices');
      return false;
    }
    ...
    if (duplicateNodes) {
      log.error('infrastructure.json contains duplicate nodes');
      return false;
    }
    ...
    if ((new Set(groupsIds)).size !== groupsIds.length) {
      log.error('infrastructure.json contains duplicate groups');
      return false;
    }
    ...
    if (!devicesExist) {
      log.error('infrastructure.json groups references non-existent devices');
      return false;
    }
    ...
    if (!nodeTypesValid) {
      log.error('infrastructure.json contains invalid node types');
      return false;
    }
