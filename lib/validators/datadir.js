...
class DataDirValidator {
  async validateOtaManifest (object, dataDir) {
    return new Promise(function (resolve) {
      ...
      object.firmwares.forEach(async function (firmware, index) {
        try {
          await fs.accessAsync(`${dataDir}/ota/bin/${firmware.name}.bin`, fs.F_OK);
        } catch (err) {
          log.error(`Cannot find firmware ${dataDir}/ota/bin/${firmware.name}.bin`);
          errors = true;
        }
